...
def get_statistics():
    ...
    # Allowed Requests (Case Insensitive)
    cursor.execute("SELECT COUNT(*) FROM logs WHERE UPPER(decision)='ALLOWED'")
    allowed = cursor.fetchone()[0]

    # Blocked Requests (Case Insensitive)
    cursor.execute("SELECT COUNT(*) FROM logs WHERE UPPER(decision)='BLOCKED'")
    blocked = cursor.fetchone()[0]

    # Connected Clients (Unique Client IPs)
...
# Automatically create table on script import
create_database()